...
class JudgingOfflineWalkthrough(unittest.TestCase):
    ...
    def test_exact_90_second_demo_path_and_reset(self):
        ...
        def cards(n):
            ...
            expect(page.locator('#result-count')).to_contain_text('Showing ' + str(n) + ' of 13 cards')
        ...
        try:
            ...
            page.get_by_role('link', name='Begin investigation', exact=True).click()
            cards(13)
            expect(page.locator('#summary-counters')).to_contain_text('13')
            capture('02-inspected-sample.png', '13 inspected pages: 3 MSC-related, 10 other; 6 Spanish GDELT-result pages and 7 external/contextual pages.')

            page.locator('#fold-confirmed').check()
            cards(12)
            expect(page.locator('#result-count')).to_contain_text('1 folded')
            capture('03-confirmed-copy-fold.png', 'One matching-text group contains two pages; folding removes one card, not an independent story.')
            page.locator('#show-unrelated').uncheck()
            cards(3)
            page.locator('#show-possible').uncheck()
            cards(0)
            expect(page.locator('#evidence-cards')).to_contain_text('No cards match')
            page.locator('#show-possible').check()
            cards(3)
            capture('04-related-pages.png', 'Three retained MSC pages remain; all retain possible shared-reporting uncertainty.')

            page.get_by_role('link', name='Replay', exact=True).click()
            expect(page.locator('[data-replay-source]')).to_have_count(3)
            expect(page.locator('#replay-scope')).to_contain_text('filters do not change')
            capture('05-story-timeline.png', 'Publisher-claimed dates, not first publication; the Chinese page is date-only.')
            ...
            capture('06-source-evidence.png', 'Cached source excerpt, date-only precision, provenance and unresolved direction details; no external link opened.')
            page.keyboard.press('Escape')
            expect(page.locator('#source-dialog')).not_to_be_visible()
            page.get_by_role('link', name='Unresolved', exact=True).click()
            ...
            capture('07-unresolved.png', 'Route conflict, attack-time uncertainty, possible common upstream reporting and incomplete coverage remain unresolved.')
